...
/* ============================================================== */
/* Timeline widget from https://codepen.io/chris-creditdesign/pen/yuFj */
/* ============================================================== */
...
$.ajax({
		url: window.location.href+"/timeline_data.html",
		// url: "table-nih.html", 
		dataType: 'text',
		success: function(data) {
				...
				/* ============================================================== */
				/* Date Format extension from http://jsfiddle.net/bladnman/nMvVj/ */
				/* ============================================================== */
				...
				function dateFormat(date1, format, options) {
					options = options ||dateFormats;
					var date = date1,
						i, len = format.length, token,
						i2, formatter,
						res = '';
					for (i=0; i<len; i++) {
				...
				var dateFormatters = {
						...
						ddd    : function(d)    { return dayNames[d.getDay()] },
						dddd: function(d)    { return dayNames[d.getDay()+7] },
						m    : function(d)    { return d.getMonth() + 1 },
						mm    : function(d)    { return zeroPad(d.getMonth() + 1) },
						mmm    : function(d)    { return monthNames[d.getMonth()] },
					   mmmm    : function(d)    { return monthNames[d.getMonth()+12] },
				...
				var item = $(data).find("table caption p").get(0);
				console.log(item)
			...
			i.e date, headline, the text, image link and credit */
				...
				for (var i = 0; i < $('.outerwrapper div[class^="event"]').length; i++) {
						...
						if (items[i].headline) {
								...
								$('.outerwrapper div[class="event-' + i + '"]')
										.append('<h3> (' + (i + 1) + ' of ' + items.length + ')</h3>');
						}

						if (items[i].date1 < items[i].date2) {
								// $('.outerwrapper div[class="event-' + i + '"]')
								// 		.append('<h4>' + getDate(items[i].dateStart) + ' - ' + items[i].dateEnd + '</h4>');\
								$('.outerwrapper div[class="event-' + i + '"]')
								 		.append('<h4> Timestamp: ' + (getDate(items[i].dateStart)).format("yyyy-mm-dd HH:MM:ss.L") +'</h4>');
						} else {
								$('.outerwrapper div[class="event-' + i + '"]')
										.append('<h4> Timestamp: ' + (getDate(items[i].dateStart)).format("yyyy-mm-dd HH:MM:ss.L") +'</h4>');
						}
						...
						// I don't need the picture for now...
						// if (items[i].img) {
						// 		$('.outerwrapper div[class="event-' + i + '"]')
				...
				to center the timeline on the selected location*/
					...
					the current location and the zoom level */
								...
								/* ticks for a day */
								else if ((timeEnd - timeBegin) > 86400000) {
									xMonthAxis.ticks(d3.time.days, 4).tickFormat(d3.time.format('%d %b'));
									xDayAxis.ticks(d3.time.days, 1);
								}
								/* ticks for a hour */
								else if ((timeEnd - timeBegin) > 3600000 ) {
									xMonthAxis.ticks(d3.time.hours, 4).tickFormat(d3.time.format('%H'));
								...
								/* ticks for half a second */
										...
										xMonthAxis.ticks(d3.time.Millisecond, 4).tickFormat(d3.time.format('%M:%S.%L'));
					...
					then animate the locations to their new position based on the updated x scale */
								...
								var colourText =	["#363636","#eeeeee","#006699","#5C7996","#222222","#EEEEEE","#F5821F","#002448"];
								switch (counter) {
										case 0:
